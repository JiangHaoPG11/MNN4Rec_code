...
class new_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding,
                ...
                category_index, subcategory_index, cand_title_length):
        # 主题级表征
...
class NRMS_GCN_KGAT_MV_mask(torch.nn.Module):
    def __init__(self, args):
        super(NRMS_GCN_KGAT_MV_mask, self).__init__()
    ...
    def forward(self,candidate_new_word_embedding, user_clicked_new_word_embedding,
        ...
        for i in range(self.sample_num):
            ...
            new_rep_one = self.new_encoder(new_word_embedding_one, new_entity_embedding_one,
                                           ...
                                           new_category_index, new_subcategory_index,cand_title_length_one
                                           ).unsqueeze(1)
            if i == 0:
                new_rep = new_rep_one
            else:
