...
def get_word_embedding(model):
    ...
    :return:
    '''
    word_index_df = pd.read_csv('df/word_index_df.csv')
    ...
    np.save('new_title_word_embedding.npy', word_embedding)
...
def get_category_embedding(model):
    ...
    :return:
    '''
    category_index_df = pd.read_csv('df/category_index_df.csv')
    ...
    np.save('new_category_embedding.npy', category_embedding)
...
def get_subcategory_embedding(model):
    ...
    :return:
    '''
    subcategory_index_df = pd.read_csv('df/subcategory_index_df.csv')
    ...
    np.save('new_subcategory_embedding.npy', subcategory_embedding)
...
if __name__ == "__main__":
    ## 加载模型
    model = gensim.models.KeyedVectors.load_word2vec_format('../data3/GoogleNews-vectors-negative300.bin.gz',binary=True)
    ##  获取词嵌入
