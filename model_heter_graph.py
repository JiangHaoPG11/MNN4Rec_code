...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding,
        ...
        graph_pool_n1 = torch.cat([graph_pool_n1, torch.zeros_like(topic_rep[:, :, : 16]), torch.zeros_like(entity_rep[:,:,:16])],dim=1)
        Apool_nn1 = torch.bmm(torch.transpose(graph_pool_n1, 2, 1),torch.bmm(heterogeneous_user_graph_A, graph_pool_n1))
        ...
        graph_pool_t1 = torch.cat([torch.zeros_like(new_rep[:, :, :3]), graph_pool_t1, torch.zeros_like(entity_rep[:, :, :3])], dim=1)
        Apool_tt1 = torch.bmm(torch.transpose(graph_pool_t1, 2, 1),torch.bmm(heterogeneous_user_graph_A, graph_pool_t1))
        ...
        graph_pool_e1 = torch.cat([torch.zeros_like(new_rep[:, :, :9]), torch.zeros_like(topic_rep[:, :, :9]), graph_pool_e1], dim=1)
        Apool_ee1 = torch.bmm(torch.transpose(graph_pool_e1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_e1))
        xpool_e1 = torch.bmm(torch.transpose(graph_pool_e1, 2, 1), graph_attention_basis)
        # 构建池化后连接矩阵和特征向量矩阵
        Apool_tn1 = torch.bmm(torch.transpose(graph_pool_t1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_n1))
        Apool_en1 = torch.bmm(torch.transpose(graph_pool_e1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_n1))
        Apool_nt1 = torch.bmm(torch.transpose(graph_pool_n1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_t1))
        Apool_et1 = torch.bmm(torch.transpose(graph_pool_e1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_t1))
        Apool_ne1 = torch.bmm(torch.transpose(graph_pool_n1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_e1))
        Apool_te1 = torch.bmm(torch.transpose(graph_pool_t1, 2, 1), torch.bmm(heterogeneous_user_graph_A, graph_pool_e1))
        ...
        graph_pool_n2 = torch.cat( [graph_pool_n2, torch.zeros_like(xpool_t1[:, :, : 1]), torch.zeros_like(xpool_e1[:, :, :1])], dim=1)
        ...
        graph_pool_t2 = torch.cat([torch.zeros_like(xpool_n1[:, :, :1]), graph_pool_t2, torch.zeros_like(xpool_e1[:, :, :1])], dim=1)
        ...
        graph_pool_e2 = torch.cat([torch.zeros_like(xpool_n1[:, :, :1]), torch.zeros_like(xpool_t1[:, :, :1]), graph_pool_e2], dim=1)
...
class heter_graph(torch.nn.Module):
    def __init__(self, args):
        super(heter_graph, self).__init__()
