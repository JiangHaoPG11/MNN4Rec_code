...
class new_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, cand_title_length):
...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, user_clicked_new_title_length , user_clicked_new_length):
        new_rep = self.new_encoder(word_embedding, user_clicked_new_title_length).unsqueeze(0)
        new_rep = F.dropout(new_rep, p=self.dropout_prob, training=self.training)
...
class NRMS_mask(torch.nn.Module):
    def __init__(self, args):
        super(NRMS_mask, self).__init__()
    ...
    def forward(self, new_word_embedding, clicked_word_embedding,
        ...
        for i in range(self.sample_num):
            ...
            new_rep_one = self.new_encoder(title_word_embedding_one, cand_title_length_one)
        ...
        for i in range(self.user_clicked_new_num):
            ...
            user_rep_one = self.user_encoder(clicked_word_embedding_one, user_clicked_new_title_length_one,
                                             user_clicked_new_length_one).unsqueeze(0)
            if i == 0:
