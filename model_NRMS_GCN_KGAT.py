...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding, neigh_entity_embedding, neigh_relation_embedding):
        new_rep = self.new_encoder(word_embedding, entity_embedding, neigh_entity_embedding, neigh_relation_embedding).unsqueeze(0)
...
class NRMS_GCN_KGAT(torch.nn.Module):
    def __init__(self, args):
        super(NRMS_GCN_KGAT, self).__init__()
    ...
    def forward(self,candidate_new_word_embedding, user_clicked_new_word_embedding,
        ...
        for i in range(self.sample_num):
            ...
            new_rep_one = self.new_encoder(new_word_embedding_one, new_entity_embedding_one,
                                           new_neigh_entity_embedding_one, new_neigh_relation_embedding_one).unsqueeze(1)
            if i == 0:
                new_rep = new_rep_one
            else:
        ...
        for i in range(self.user_clicked_new_num):
            ...
            user_rep_one = self.user_encoder(clicked_new_word_embedding_one, clicked_new_entity_embedding_one,
                                             clicked_new_neigh_entity_embedding_one, clicked_new_neigh_relation_embedding_one).unsqueeze(0)
            if i == 0:
