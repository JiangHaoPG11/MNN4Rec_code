...
class new_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding, cand_title_length):
        # 单词级新闻表征
        ...
        entity_inter = self.norm1(entity_inter)
...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding, user_clicked_new_title_length, user_clicked_new_length):
        new_rep = self.new_encoder(word_embedding, entity_embedding,user_clicked_new_title_length).unsqueeze(0)
...
class NRMS_GCN_mask(torch.nn.Module):
    def __init__(self, args):
        super(NRMS_GCN_mask, self).__init__()
        ...
        self.new_encoder = new_encoder(self.word_dim, self.attention_dim, self.attention_heads,
                                       self.query_vector_dim, self.entity_size, self.entity_embedding_dim)
        self.user_encoder = user_encoder(self.word_dim, self.attention_dim, self.attention_heads,
                                         self.query_vector_dim, self.entity_size, self.entity_embedding_dim)
    ...
    def forward(self, candidate_new_word_embedding, user_clicked_new_word_embedding,
        ...
        for i in range(self.sample_num):
            ...
            new_rep_one = self.new_encoder(new_word_embedding_one, new_entity_embedding_one, cand_title_length_one)
