...
class new_encoder(torch.nn.Module):
    def __init__(self, word_dim, attention_dim, attention_heads, query_vector_dim, entity_size, entity_embedding_dim,
        ...
        # 新闻注意力
        self.new_attention = QueryAttention(query_vector_dim, self.multi_dim)
    ...
    def forward(self, word_embedding, entity_embedding,
                ...
                category_index, subcategory_index, cand_title_length, user_vector):
        # 主题级表征
        ...
        entity_rep = torch.tanh(self.entity_attention(user_vector, entity_inter))
        ...
        loss = self.new_rep_loss(new_rep, category_index, self.category_size)
        return new_rep, loss

    def new_rep_loss(self, new_rep, category_index, category_size):
        category = torch.IntTensor(np.linspace(0, category_size-1,num=category_size))
        category_embedding = self.embedding_layer1(category)
        category_rep = self.fc1(category_embedding)
        rep_score = torch.sum(new_rep.unsqueeze(1) *
                              category_rep.unsqueeze(0).repeat(new_rep.shape[0],1,1), dim = 2)
        loss = torch.nn.functional.cross_entropy(rep_score, category_index.long())
        return loss.item()
...
class user_encoder(torch.nn.Module):
    def __init__(self, word_dim, attention_dim, attention_heads, query_vector_dim, entity_size, entity_embedding_dim,
        ...
        self.user_attention = QueryAttention(query_vector_dim, self.multi_dim)
    ...
    def forward(self, word_embedding, entity_embedding,
                ...
                user_clicked_new_title_length, user_clicked_new_length, user_vector_1, user_vector_2):
        # 点击新闻表征
        new_rep, loss = self.new_encoder(word_embedding, entity_embedding,
                                        neigh_entity_embedding, neigh_relation_embedding,
                                        category_index, subcategory_index,
                                        user_clicked_new_title_length, user_vector_1.unsqueeze(1).repeat(50,1,1))
        new_rep = new_rep.unsqueeze(0)
...
class exp2(torch.nn.Module):
    ...
    def forward(self,candidate_new_word_embedding, user_clicked_new_word_embedding,
        ...
        for i in range(self.sample_num):
            ...
            new_rep_one, loss = self.new_encoder(new_word_embedding_one, new_entity_embedding_one,
                                                 new_neigh_entity_embedding_one, new_neigh_relation_embedding_one,
                                                 new_category_index, new_subcategory_index,cand_title_length_one, user_vector_1)
        ...
        for i in range(self.user_clicked_new_num):
            ...
            # 用户表征
            user_rep_one, loss = self.user_encoder(clicked_new_word_embedding_one, clicked_new_entity_embedding_one,
                                             ...
                                             user_clicked_new_title_length_one, user_clicked_new_length_one, user_vector_1_one, user_vector_2_one)
        ...
        new_loss = np.array(loss_list).mean()
        return sorce, new_loss
    ...
    def loss(self, candidate_new_word_embedding,  user_clicked_word_embedding,
        ...
        sorce,new_loss = self.forward(candidate_new_word_embedding, user_clicked_word_embedding,
                             candidate_new_entity_embedding, user_clicked_new_entity_embedding,
                             candidate_new_neigh_entity_embedding,user_clicked_new_neigh_entity_embedding,
                             candidate_new_neigh_relation_embedding, user_clicked_new_neigh_relation_embedding,
                             candidate_new_category_index, user_clicked_new_category_index,
