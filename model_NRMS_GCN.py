...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding):
        new_rep = self.new_encoder(word_embedding, entity_embedding).unsqueeze(0)
...
class NRMS_GCN(torch.nn.Module):
    def __init__(self, args):
        super(NRMS_GCN, self).__init__()
    ...
    def forward(self, candidate_new_word_embedding, user_clicked_new_word_embedding,
        ...
        for i in range(self.user_clicked_new_num):
            ...
            user_rep_one = self.user_encoder(clicked_word_embedding_one, clicked_entity_embedding_one).unsqueeze(0)
            if i == 0:
