...
class new_encoder(torch.nn.Module):
    def __init__(self, word_dim, attention_dim, attention_heads, query_vector_dim, entity_size, entity_embedding_dim,
        ...
        self.word_attention = QueryAttention_2(query_vector_dim, self.multi_dim)
        ...
        self.entity_attention = QueryAttention_2(query_vector_dim, self.multi_dim)
        ...
        # 新闻注意力
        self.new_attention = Additive_Attention(query_vector_dim, 2 * self.multi_dim)
        self.norm8 = nn.LayerNorm(2 * self.multi_dim)
    ...
    def forward(self, word_embedding, entity_embedding,
        ...
        # 用户长期兴趣感知的新闻表征
        new_rep_Long = torch.cat([word_rep, entity_rep], dim=1)
        # 用户短球兴趣感知的新闻表征
        new_rep_short = torch.cat([word_rep_c, entity_rep_c], dim=1)
        # 新闻附加注意力
        new_rep = torch.cat([new_rep_Long.unsqueeze(1), new_rep_short.unsqueeze(1)], dim=1)
...
class user_encoder(torch.nn.Module):
    def __init__(self, word_dim, attention_dim, attention_heads, query_vector_dim, entity_size, entity_embedding_dim,
        ...
        self.new_encoder = new_encoder(word_dim, attention_dim, attention_heads, query_vector_dim, entity_size, entity_embedding_dim,
                                       category_dim, category_size)
        self.multiheadatt = MultiHeadSelfAttention_2(2 * attention_dim * attention_heads, 2 * attention_dim * attention_heads, attention_heads)
        self.multi_dim = attention_dim * attention_heads
        self.norm1 = nn.LayerNorm(2 * self.multi_dim)
        self.user_attention_n = QueryAttention_2(2 * self.multi_dim, 2 * self.multi_dim)
        self.user_attention = Additive_Attention(query_vector_dim, 2 * self.multi_dim)
        self.attention = Additive_Attention(query_vector_dim, 2 * self.multi_dim)
        self.norm2 = nn.LayerNorm(2 * self.multi_dim)
        self.dropout_prob = 0.2

    def forward(self, word_embedding, entity_embedding,
                neigh_entity_embedding, neigh_relation_embedding,
                category_index, user_clicked_new_title_length, user_clicked_new_length, user_vector_1, candidate_new_vector):
        # 点击新闻表征
        ...
        # user_rep = self.user_attention(new_rep).unsqueeze(1)
        # 用户表征
        for i in range(5):
            ...
            user_rep_one = self.user_attention(new_rep).unsqueeze(1)
        ...
        user_rep = self.attention(user_rep_cat).unsqueeze(0)
        user_rep = self.norm2(user_rep)
        return user_rep

class exp5(torch.nn.Module):
    '''
        新闻编码器： 将主题信息作为注意力的询问向量，分别选择单词和实体，构建主题感知，作为长期兴趣（单词和实体）
                   将用户向量作为注意力的询问向量，分别选择单词和实体，构建非主题感知，作为短期兴趣（单词和实体）
        用户编码器： 分别构建候选新闻感知和非候选新闻感知的用户表征
     '''
    ...
    def forward(self,candidate_new_word_embedding, user_clicked_new_word_embedding,
        ...
        for i in range(self.batch_size):
            ...
            user_rep_one = self.user_encoder(clicked_new_word_embedding_one, clicked_new_entity_embedding_one,
                                             ...
                                             user_vector_1_one, candidate_new_vector)
            if i == 0:
                user_rep = user_rep_one
            else:
