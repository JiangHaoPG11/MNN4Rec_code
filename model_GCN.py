...
class new_encoder(torch.nn.Module):
    def __init__(self,  attention_dim, query_vector_dim, entity_size, entity_embedding_dim):
        ...
        self.GCN = gcn(entity_size, entity_embedding_dim, attention_dim)
...
class user_encoder(torch.nn.Module):
    ...
    def forward(self,  entity_embedding):
        new_rep = self.new_encoder(entity_embedding).unsqueeze(0)
...
class GCN(torch.nn.Module):
    ...
    def forward(self, candidate_new_entity_embedding, user_clicked_new_entity_embedding):
        ...
        for i in range(self.sample_num):
            ...
            new_rep_one = self.new_encoder(new_entity_embedding_one)
        ...
        for i in range(self.user_clicked_new_num):
            ...
            user_rep_one = self.user_encoder(clicked_entity_embedding_one).unsqueeze(0)
            if i == 0:
    ...
    def loss(self, candidate_new_entity_embedding, user_clicked_new_entity_embedding, label):
